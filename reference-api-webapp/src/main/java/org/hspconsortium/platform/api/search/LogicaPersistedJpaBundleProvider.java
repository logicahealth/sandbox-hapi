...
public class LogicaPersistedJpaBundleProvider implements IBundleProvider {
    private static final Logger ourLog = LoggerFactory.getLogger(LogicaPersistedJpaBundleProvider.class);
    ...
    public LogicaPersistedJpaBundleProvider(RequestDetails theRequest, String theSearchUuid, IDao theDao, SearchBuilderFactory theSearchBuilderFactory) {
    ...
    public Integer size() {
        ensureSearchEntityLoaded();
        LogicaSearchCoordinatorSvcImpl.verifySearchHasntFailedOrThrowInternalErrorException(mySearchEntity);
